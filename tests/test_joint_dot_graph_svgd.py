...
if __name__ == '__main__':
    ...
    '''SVGD'''
    tuned = joint_config[generative_model_str][n_vars if n_vars in joint_config['lingauss'].keys() else 20]
    ...
    def linear_alpha(t):
        return (tuned.get('alpha_linear', 1.0) * t) + tuned.get('alpha', 0.0)

    def linear_beta(t):
        return (tuned.get('beta_linear', 1.0) * t) + tuned.get('beta', 0.0)
    ...
    if graph_embedding_representation:
        ...
        init_particles_x = random.normal(subk, shape=(n_particles, n_vars, n_dim, 2)) * latent_prior_std
    else:
        latent_prior_std = 1.0
        init_particles_x = random.normal(subk, shape=(n_particles, n_vars, n_vars)) * latent_prior_std
    print('init_particles_x    ', init_particles_x.shape)

    key, subk = random.split(key)
    init_particles_theta = model.init_parameters(key=subk, n_particles=n_particles, n_vars=n_vars)
    ...
    # metrics
    metrics = svgd.make_metrics(x_ho=x_ho, eltwise_log_likelihood=eltwise_log_likelihood, target=target)
    ...
    '''Analysis'''
    topk = 10
    svgd_g = svgd.particle_to_hard_g(particles_x)
    svgd_empirical = particle_joint_empirical(svgd_g, particles_theta)
    svgd_ids, svgd_thetas, svgd_logprobs = svgd_empirical

    # number of cyclic graphs returned by SVGD
    svgd_unique_g = id2bit(onp.unique(svgd_ids, axis=0), n_vars)
    dag_count = (elwise_acyclic_constr_nograd(svgd_g, n_vars) == 0).sum()
    unique_dag_count = (elwise_acyclic_constr_nograd(svgd_unique_g, n_vars) == 0).sum()

    print(
        '\nSVGD Acyclicity\n' + \
        'graphs : {}\t'.format(svgd_g.shape[0]) +
        'unique graphs : {}\n'.format(svgd_unique_g.shape[0]) +
        'cyclic graphs :        {}\t'.format(svgd_g.shape[0] - dag_count) +
        'percentage cyclic :        {:6.04f} %\n'.format(100 * (svgd_g.shape[0] - dag_count) / n_particles) +
        'cyclic unique graphs : {}\t'.format(svgd_unique_g.shape[0] - unique_dag_count) +
        'percentage unique cyclic : {:6.04f} %\n'.format(100 * (svgd_unique_g.shape[0] - unique_dag_count) / svgd_ids.shape[0])
    )
